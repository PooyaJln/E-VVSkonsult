...
export const itemsReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case itemsActionTypes.DELETE_ITEM:
      ...
      return {
        ...state,
        items: _items.filter((item) => item.room_id !== action.payload.room_id),
      };
    case itemsActionTypes.UPDATE_ITEM:
      _items = state.items.map((item) => {
        return item.room_id === action.payload.room_id ? action.payload : item;
      });
      return {
        items: _items,
      };
...
export const RoomsContextProvider = ({ children }) => {
  ...
  let URI = "http://localhost:4001/heat-loss/rooms/";
  ...
  apiCalls.updateItem = async (id, data) => {
    try {
      let itemsNamesList = [];
      state.items.map((item) => itemsNamesList.push(item.room_name));
      if (itemsNamesList.includes(data.room_name)) {
  ...
  return (
    <RoomsContext.Provider value={{ state, dispatch, uiCalls, apiCalls }}>
      {children}
