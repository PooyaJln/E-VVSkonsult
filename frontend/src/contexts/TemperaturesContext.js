  ...
  switch (action.type) {
    ...
    case itemsActionTypes.DELETE_ITEM:
      ...
      return {
        ...state,
        items: _items.filter(
          (item) => item.temperature_id !== action.payload.temperature_id
        ),
      };
    case itemsActionTypes.UPDATE_ITEM:
      _items = state.items.map((item) => {
        return item.temperature_id === action.payload.temperature_id
          ? action.payload
          : item;
      });
      return {
...
export const TemperaturesContext = createContext();

export const TemperaturesContextProvider = ({ children }) => {
  const [state, dispatch] = useReducer(itemsReducer, itemsInitialState);

  let URI = "http://localhost:4001/heat-loss/temperatures/";
