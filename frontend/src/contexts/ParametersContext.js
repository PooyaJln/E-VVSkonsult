  ...
  switch (action.type) {
    ...
    case itemsActionTypes.DELETE_ITEM:
      ...
      return {
        ...state,
        items: _items.filter(
          (item) => item.parameter_id !== action.payload.parameter_id
        ),
      };
    case itemsActionTypes.UPDATE_ITEM:
      _items = state.items.map((item) => {
        return item.parameter_id === action.payload.parameter_id
          ? action.payload
          : item;
      });
      return {
...
export const ParametersContextProvider = ({ children }) => {
  const [state, dispatch] = useReducer(itemsReducer, {
    items: itemsInitialState.items,
  });

  let URI = "http://localhost:4001/heat-loss/thermal-parameters/";
  ...
  apiCalls.updateItem = async (id, data) => {
    try {
      let itemsNamesList = [];
      state.items.map((item) => itemsNamesList.push(item.parameter_name));
      if (itemsNamesList.includes(data.parameter_name)) {
  ...
  return (
    <ParametersContext.Provider value={{ state, dispatch, uiCalls, apiCalls }}>
      {children}
