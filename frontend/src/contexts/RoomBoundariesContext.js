...
export const itemsReducer = (state, action) => {
  ...
  switch (action.type) {
    case itemsActionTypes.GET_ITEMS:
      return {
        ...state,
        items: action.payload.boundaries,
      };
    ...
    case itemsActionTypes.DELETE_ITEM:
      ...
      return {
        ...state,
        items: _items.filter(
          (item) => item.boundary_id !== action.payload.boundary_id
        ),
      };
    case itemsActionTypes.UPDATE_ITEM:
      _items = state.items.map((item) => {
        return item.boundary_id === action.payload.boundary_id
          ? action.payload
          : item;
      });
      return {
...
export const RoomBoundariesContext = createContext();

export const RoomBoundariesContextProvider = ({ children }) => {
  const [state, dispatch] = useReducer(itemsReducer, itemsInitialState);

  let URI = "http://localhost:4001/heat-loss/roomBoundaries/";
  ...
  apiCalls.updateItem = async (id, data) => {
    try {
      let itemsNamesList = [];
      state.items.map((item) => itemsNamesList.push(item.boundary_name));
      if (itemsNamesList.includes(data.boundary_name)) {
  ...
  return (
    <RoomBoundariesContext.Provider
      value={{ state, dispatch, uiCalls, apiCalls }}
    >
      {children}
    </RoomBoundariesContext.Provider>
  );
